...
function DsrFormFields({ form, companySlot }) {
  return (
    <Stack gap="md">
      <div>
      ...
      <div>
        ...
        <Textarea label="Remarks" autosize minRows={2} {...form.getInputProps('remarks')} />
      </div>
    </Stack>
  );
}
...
export default function DsrPage() {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: (info) => {
          const row = info.row.original;
          if (!canEdit || String(row.agentId?._id) !== String(user.id) || row.convertedToPipeline) {
            return <Badge color={STATUS_COLOR[row.status] || 'gray'} variant="light">{row.status}</Badge>;
          }
          return (
            <Select
        ...
        cell: (info) => {
          ...
          return (
            <Group gap="xs" wrap="nowrap">
              {row.status === 'Interested' && !row.convertedToPipeline && (
              ...
              <Tooltip label="Chat about this DSR (tag teammates, see full history)">
                <Indicator
                  ...
                  offset={4}
                >
                  <ActionIcon variant="light" size="lg" radius="md" onClick={() => openChat(row.dsrNo)} aria-label="Chat">
                    <MessageCircle size={18} />
                  </ActionIcon>
                </Indicator>
              </Tooltip>
              {(isOwner || isAdmin) && !row.convertedToPipeline && (
                <Tooltip label="Edit this DSR record (company, contact, remarks, status)">
                  <ActionIcon variant="light" size="lg" radius="md" onClick={() => openEdit(row)} aria-label="Edit DSR">
                    <Pencil size={18} />
                  </ActionIcon>
                </Tooltip>
              )}
              {(isOwner || isAdmin) && row.convertedToPipeline && (
                <Tooltip label="This DSR moved to the Sales Pipeline and can no longer be edited here">
                  <ActionIcon variant="light" size="lg" radius="md" disabled aria-label="Edit disabled">
                    <Pencil size={18} />
                  </ActionIcon>
                </Tooltip>
              )}
  ...
  return (
    <Stack>
      ...
      <Modal opened={!!editRow} onClose={() => setEditRow(null)} title={`Edit DSR — ${editRow?.dsrNo || ''}`} size="lg">
        <form onSubmit={editForm.onSubmit(handleEdit)}>
          <DsrFormFields
            form={editForm}
            companySlot={<TextInput label="Company" required leftSection={<Building2 size={16} />} {...editForm.getInputProps('company')} />}
          />
          <Button type="submit" mt="md" fullWidth>Save changes</Button>
        </form>
      </Modal>
    </Stack>
  );
